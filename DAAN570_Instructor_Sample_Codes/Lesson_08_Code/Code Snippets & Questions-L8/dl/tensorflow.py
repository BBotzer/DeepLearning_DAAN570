# This file is generated is based on the d2lbook lib
...
def set_figsize(figsize=(3.5, 2.5)):
    ...
    use_svg_display()
    dl.plt.rcParams['figure.figsize'] = figsize
...
def plot(X, Y=None, xlabel=None, ylabel=None, legend=None, xlim=None,
    ...
    set_figsize(figsize)
    axes = axes if axes else dl.plt.gca()
...
def synthetic_data(w, b, num_examples):
    ...
    X = dl.zeros((num_examples, w.shape[0]))
    X += tf.random.normal(shape=X.shape)
    y = dl.matmul(X, tf.reshape(w, (-1, 1))) + b
...
def linreg(X, w, b):
    ...
    return dl.matmul(X, w) + b


# Defined in file: ./chapter_linear-networks/linear-regression-scratch.md
def squared_loss(y_hat, y):
    """Squared loss."""
    return (y_hat - dl.reshape(y, y_hat.shape))**2 / 2
...
def accuracy(y_hat, y):
    ...
    if len(y_hat.shape) > 1 and y_hat.shape[1] > 1:
        y_hat = dl.argmax(y_hat, axis=1)
    cmp = dl.astype(y_hat, y.dtype) == y
    return float(dl.reduce_sum(dl.astype(cmp, y.dtype)))
...
def evaluate_accuracy(net, data_iter):
    ...
    for X, y in data_iter:
        metric.add(accuracy(net(X), y), dl.size(y))
...
def predict_ch3(net, test_iter, n=6):
    ...
    preds = dl.get_fashion_mnist_labels(dl.argmax(net(X), axis=1))
    titles = [true + '\n' + pred for true, pred in zip(trues, preds)]
    dl.show_images(dl.reshape(X[0:n], (n, 28, 28)), 1, n,
                    titles=titles[0:n])
...
def evaluate_loss(net, data_iter, loss):
    ...
    metric = dl.Accumulator(2)  # Sum of losses, no. of examples
    for X, y in data_iter:
        l = loss(net(X), y)
        metric.add(dl.reduce_sum(l), dl.size(l))
    return metric[0] / metric[1]
...
def seq_data_iter_sequential(corpus, batch_size, num_steps):
    ...
    Xs = dl.tensor(corpus[offset:offset + num_tokens])
    Ys = dl.tensor(corpus[offset + 1:offset + 1 + num_tokens])
    Xs = dl.reshape(Xs, (batch_size, -1))
    Ys = dl.reshape(Ys, (batch_size, -1))
...
class SeqDataLoader:
    ...
    def __init__(self, batch_size, num_steps, use_random_iter, max_tokens):
        if use_random_iter:
            self.data_iter_fn = dl.seq_data_iter_random
        else:
            self.data_iter_fn = dl.seq_data_iter_sequential
        self.corpus, self.vocab = dl.load_corpus_time_machine(max_tokens)
...
# Defined in file: ./chapter_recurrent-neural-networks/rnn-scratch.md
def predict_dl(prefix, num_preds, net, vocab):
    """Generate new characters following the `prefix`."""
    state = net.begin_state(batch_size=1, dtype=tf.float32)
    outputs = [vocab[prefix[0]]]
    get_input = lambda: dl.reshape(dl.tensor([outputs[-1]]), (1, 1)).numpy()
...
# Defined in file: ./chapter_recurrent-neural-networks/rnn-scratch.md
def train_epoch_dl(net, train_iter, loss, updater, use_random_iter):
    ...
    for X, Y in train_iter:
        ...
        # Keras loss by default returns the average loss in a batch
        # l_sum = l * float(dl.size(y)) if isinstance(
        #     loss, tf.keras.losses.Loss) else tf.reduce_sum(l)
        metric.add(l * dl.size(y), dl.size(y))
    return math.exp(metric[0] / metric[1]), metric[1] / timer.stop()


# Defined in file: ./chapter_recurrent-neural-networks/rnn-scratch.md
def train_dl(net, train_iter, vocab, lr, num_epochs, strategy,
    ...
    predict = lambda prefix: predict_dl(prefix, 50, net, vocab)
    # Train and predict
    for epoch in range(num_epochs):
        ppl, speed = train_epoch_dl(net, train_iter, loss, updater,
                                     use_random_iter)
        if (epoch + 1) % 10 == 0:
...
def build_array_nmt(lines, vocab, num_steps):
    ...
    valid_len = dl.reduce_sum(dl.astype(array != vocab['<pad>'], dl.int32),
                               1)
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            gradients = dl.grad_clipping(gradients, 1)
...
def show_heatmaps(matrices, xlabel, ylabel, titles=None, figsize=(2.5, 2.5),
    ...
    fig, axes = dl.plt.subplots(num_rows, num_cols, figsize=figsize,
                                 sharex=True, sharey=True, squeeze=False)
    for i, (row_axes, row_matrices) in enumerate(zip(axes, matrices)):
        for j, (ax, matrix) in enumerate(zip(row_axes, row_matrices)):
            pcm = ax.imshow(dl.numpy(matrix), cmap=cmap)
            if i == num_rows - 1:
...
def masked_softmax(X, valid_lens):
    ...
    if valid_lens is None:
        ...
        X = dl.sequence_mask(tf.reshape(X, shape=(-1, shape[-1])),
...
class AttentionDecoder(dl.Decoder):
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        self.attention = dl.DotProductAttention(dropout)
...
class EncoderBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens,
                 ...
                 **kwargs):
        super().__init__(**kwargs)
        self.attention = dl.MultiHeadAttention(key_size, query_size,
...
# Defined in file: ./chapter_attention-mechanisms/transformer.md
class TransformerEncoder(dl.Encoder):
    """Transformer encoder."""
    def __init__(self, vocab_size, key_size, query_size, value_size,
        ...
        self.pos_encoding = dl.PositionalEncoding(num_hiddens, dropout)
        self.blks = [
...
def show_trace_2d(f, results):
    ...
    x1, x2 = dl.meshgrid(dl.arange(-5.5, 1.0, 0.1),
                          dl.arange(-3.0, 1.0, 0.1))
...
def box_corner_to_center(boxes):
    ...
    w = x2 - x1
    h = y2 - y1
    boxes = dl.stack((cx, cy, w, h), axis=-1)
    return boxes
...
def box_center_to_corner(boxes):
    ...
    x2 = cx + 0.5 * w
    y2 = cy + 0.5 * h
    boxes = dl.stack((x1, y1, x2, y2), axis=-1)
    return boxes
...
def bbox_to_rect(bbox, color):
    ...
    return dl.plt.Rectangle(xy=(bbox[0], bbox[1]), width=bbox[2] - bbox[0],
...
dl.DATA_HUB['pokemon'] = (dl.DATA_URL + 'pokemon.zip',
